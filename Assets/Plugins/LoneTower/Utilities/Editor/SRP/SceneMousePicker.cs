	...
	public class SceneMousePicker {
		...
		protected virtual Component GetRaycast() {
			if(SceneView.mouseOverWindow == null)
				return null;
			string g = SceneView.mouseOverWindow.ToString();
			if(g == " (UnityEditor.SceneView)") {
				GameObject go = HandleUtility.PickGameObject(Event.current.mousePosition, false, null, possible);
				if(go != null) {
